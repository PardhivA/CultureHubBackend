...
export default function Followbox({ follower, reFetch }) {
  ...
  const { data1 } = useFetch1(
    `https://culture1.onrender.com/api/user/${user.id}/follow/${follower._id}`
  );
  ...
  //     e.preventDefault();
  //     try{
  //     const res=await axios.put(`https://culture1.onrender.com/api/user/${follower._id}/follow/`,flw);
  //     if(res){
  //         setsuc("success");
  //         reFetch();
  ...
  //     e.preventDefault();
  //     try{
  //     const res=await axios.put(`https://culture1.onrender.com/api/user/${follower._id}/unfollow/`,flw);
  //     if(res){
  //         setsuc("success");
  //         reFetch();
    ...
    if (followText == "Follow") {
      try {
        const res = await axios.put(
          `https://culture1.onrender.com/api/user/${follower._id}/follow/`,
          flw
        );
        if (res) {
          setsuc("success");
      ...
      try {
        const res = await axios.put(
          `https://culture1.onrender.com/api/user/${follower._id}/unfollow/`,
          flw
        );
        if (res) {
          setsuc("success");
  ...
  return (
    <div className="follower">
      ...
      <button
        ...
        onClick={handleclick}
        style={{
          backgroundColor: "linear-gradient(to right, #0074e4, #00a1ff);",
          position: "fixed",
        }}
      >
        {followText}
