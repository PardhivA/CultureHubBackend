...
export default function Posts({ item, margin, height, reload }) {
  ...
  const { data, loading, reFetch } = useFetch(
    `https://culture1.onrender.com/api/post/${item._id}/comments`
  );
  ...
  const handlecomment = async () => {
    try {
      ...
      const res = await axios.post(
        `https://culture1.onrender.com/api/post/${item._id}/comment`,
        addcomment
      );
      setsuc("success");
      reFetch();
  ...
  const { data2, reFetch2 } = useFetch2(
    `https://culture1.onrender.com/api/user/${item.userId}`
  );

  const { data1, reFetch1 } = useFetch1(
    `https://culture1.onrender.com/api/post/${item._id}/like/${user.id}`
  );
  //console.log(data1);
  ...
  const handlelike = async (e) => {
    ...
    try {
      const res = await axios.put(
        `https://culture1.onrender.com/api/post/${item._id}/like`,
        liked
      );
      if (res) setsuc("liked");
      reFetch1();
  ...
  const handldelete = async (e) => {
    ...
    try {
      const res = await axios.delete(
        `https://culture1.onrender.com/api/post/${item._id}`,
        liked
      );
      if (res) reload();
    } catch (e) {
