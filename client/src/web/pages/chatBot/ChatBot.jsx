...
const ChatBot = () => {
  ...
  const translateText = async (text, targetLanguage) => {
    try {
      const apiKey = "AIzaSyAb_c3thDz8zsgkFU-s5OKhqLUBDpgGfbY";
  ...
  const handleTTSreq = async () => {
    ...
    try {
      ...
      const url = `https://culture1.onrender.com/api/tts/${selectedLanguage}`;
  ...
  const handleTTSreqFEMALE = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        "https://culture1.onrender.com/api/tts/female",
        val
      );
      console.log(res);
      const audioBase64 = res.data.audioResponse;
